...
python37Packages.buildPythonApplication rec {
  ...
  postPatch = ''
    ...
    substituteInPlace docs/conf.py --subst-var-by version ${version}
  '';

  nativeBuildInputs = [ pkgconfig ];
  buildInputs = [ glib libxcb cairo pango python37Packages.xcffib ];

  pythonPath = with python37Packages; [ xcffib psutil cairocffi-xcffib setuptools ];
